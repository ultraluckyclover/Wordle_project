...
#bucket = get_word_buckets('train', SHORT_WORD_LIST_FILE)
#print(bucket)

#dist = get_pattern_distributions('train', possible_words, get)[0]
...
def get_weights(words, priors):
    ...
    #print("weight is ", frequencies/total)
...
#Drew added this to show the words with the most entropy

    e = dict(zip(possible_words,H1s))
    sort_it = sorted(e.items(),key = lambda x:x[1], reverse = True)
...
def simulate_games(first_guess=None,
    ...
    if first_guess is None:
        first_guess = optimal_guess(
            all_words, short_word_list, priors
        )
